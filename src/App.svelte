<script lang="ts">
  ...
  const check = () => {
    결과.월급 = Math.round(연봉 / 12)
    결과.시급 = Math.round(결과.월급 / (근무시간 + 야간*1.5 + 주말*2 + 휴가*1.5))
    결과.기본급 = Math.round(결과.시급 * 근무시간 - 복리후생비)
    결과.소계 = 결과.기본급 + 복리후생비
    결과.야간 = Math.round(결과.시급 * 야간 * 1.5)
    결과.주말 = Math.round(결과.시급 * 주말 * 2)
    결과.휴가 = Math.round(결과.시급 * 휴가 * 1.5)
    미결과.기본급 = 결과.기본급
    미결과.소계 = 미결과.기본급 + 복리후생비 - Math.round(9620*근무시간/100)
    미결과.시급 = Math.round(미결과.소계 / 226)
...
<main>
  ...
  {#if 결과.시급 != 0}
  <div>복리후생비 포함</div>
    <div>월급: {결과.월급}</div><div>시급: {결과.시급}</div> <div>기본급: {결과.기본급}</div> <div>소계(기본급+복리후생비): {결과.소계}</div> <div>야간: {결과.야간}</div> <div>주말: {결과.주말}</div> <div>휴가: {결과.휴가}</div>
    <hr>
  {/if}
  {#if 미결과.시급 != 0}
  <div>복리후생비 {Math.round(9620*근무시간/100)} 미산입 계산 </div>
<div>시급: {미결과.시급}</div> <div>기본급: {미결과.기본급}</div> <div>소계: {미결과.소계}</div>
  {/if}
